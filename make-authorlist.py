...
def format_prl(authorlist, institutes):
...
def format_epjc(authorlist, institutes):
    '''Format for EPJC'''
    print_header()
...
def format_jcap(authorlist, institutes):
...
def format_ptep(authorlist, institutes):
    '''Format for PTEP '''
    print_header()
    print(r'''
...
def format_arxiv(authorlist, institutes):
...
def read_authorlist(filename):
    ...
    rx_instaddr = re.compile(r'(?P<institute>.*)\s\@\s(?P<address>.*)\n')
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            match_institute = rx_instaddr.search(line)
            if match:
                author = { }
            ...
            elif match_institute:
                institute = match_institute.group("institute")
                address = match_institute.group("address")
                institutes[institute] = address
            else:
                print("Failed on:", line)
                sys.exit(1)
    return authorlist, institutes

def check_authorlist(authorlist, institutes):
    for author in authorlist:
        for inst in author['institute']:
            if institutes[inst] == None:
                print("No institute defined for", author['name'])
            if author['alsoat']:
                if institutes[author['alsoat']] == None:
                    print("No institute defined for", author['name'])
...
if __name__ == "__main__":
    ...
    option = sys.argv[1]
    filename = sys.argv[2]
    authorlist, institutes = read_authorlist(filename)
    check_authorlist(authorlist, institutes)
    try:        
        options[option](authorlist, institutes)
    except KeyError:
        usage()
